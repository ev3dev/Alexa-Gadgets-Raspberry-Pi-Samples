...
ObjectManagerName = 'org.freedesktop.DBus.ObjectManager'
...
class _BlueZAPI(dbus.service.Object):
    """
    ...
    def __init__(self):
        ...
        self._bluez_adapter = self._create_bluez_adapter()

        # initialize bluez properties
        self._bluez_properties = dbus.Interface(self._bus.get_object(BUS_NAME, self._bluez_adapter.object_path), PropertiesName)
    ...
    def _create_bluez_adapter(self):
        # look for the path of bluez adapter, e.g. /org/bluez/hci0
        objs = dbus.Interface(self._bus.get_object(BUS_NAME, '/'), ObjectManagerName).GetManagedObjects()
        for path, _interface in objs.items():
            adapter = _interface.get(ADAPTER_INTERFACE)
            if adapter is None:
                continue
            # use the first adapter which is the default adatper
            # this means we only support one bt adapter
            return dbus.Interface(self._bus.get_object(BUS_NAME, path), ADAPTER_INTERFACE)
        return None
    ...
    def _find_device(self, bd_addr):
        ...
        for path, _interface in objs.items():
            ...
            if device['Address'] == bd_addr:
                return dbus.Interface(self._bus.get_object(BUS_NAME, path), DEVICE_INTERFACE)
        return None
    ...
    def unpair(self, bd_addr):
        ...
        if device is not None:
            self._bluez_adapter.RemoveDevice(device.object_path)
        else:
